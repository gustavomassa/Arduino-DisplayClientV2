...
void TM1637::o_digitalWrite(uint8_t pin, uint8_t val){
...
void TM1637::writeByte(int8_t wr_data)
{
  ...
  {
    o_digitalWrite(Clkpin,LOW);      
    if(wr_data & 0x01)o_digitalWrite(Datapin,HIGH);//LSB first
    else o_digitalWrite(Datapin,LOW);
	delayMicroseconds(3);
    wr_data >>= 1;      
    o_digitalWrite(Clkpin,HIGH);
	delayMicroseconds(3);
      
  }  
  o_digitalWrite(Clkpin,LOW); //wait for the ACK
...
{
  o_digitalWrite(Clkpin,HIGH);//send start signal to TM1637
  o_digitalWrite(Datapin,HIGH);
 // delayMicroseconds(2);
  o_digitalWrite(Datapin,LOW); 
  o_digitalWrite(Clkpin,LOW); 
...
void TM1637::rawDisplay(uint8_t BitAddr, int8_t DispData)
{
	int8_t SegData;
	SegData = (byte)DispData;
	start();          //start signal sent to TM1637 from MCU
	writeByte(ADDR_FIXED);//
	stop();           //
	start();          //
